...
pub mod Login {
    ...
    impl LoginImpl of super::ILogin<ContractState> {
        ...
        ///
        /// This function can be reached by a user with a valid ZK proof.
        /// The function changes user's public key (and expiration time) to the new one given
        /// in the transaction, then tries to collect any existing debt.
        ///
...
//            println!("Deployed address {:?}", address);
    ...
    pub impl PrivateImpl of IPrivate {
        ...
        /// Tries to collect user's debt internally during login.
        ///
        /// If the user has debt and sufficient balance, collects the debt.
        /// If balance is insufficient, skips collection without blocking login.
        /// The debt will be collected on the next transaction via Account.__execute__.
        fn try_collect_debt_internal(ref self: ContractState, user_address: ContractAddress) {
            let debt = self.user_debt.entry(user_address).read();

            // No debt, nothing to collect
            if debt == 0 {
                return;
            }

            // Check if user has sufficient balance (uses 2x debt as safety margin)
            if !user_can_repay(user_address, debt) {
                // Insufficient balance, skip collection
                // Debt will be collected on next transaction
                return;
            }

            // Call Account.pay() to collect debt
            syscalls::call_contract_syscall(
                user_address,
                selector!("pay"),
                array![].span()
        ...
        fn garaga_verify_get_public_inputs(self: @ContractState, calldata: Span<felt252>) ->  Span<u256> {
            let mut _res = syscalls::library_call_syscall(
                ...
                .unwrap_syscall();
            // Garaga v1.0.1 returns Result<Span<u256>, felt252>
            let result = Serde::<Result<Span<u256>, felt252>>::deserialize(ref _res).unwrap();
            match result {
                Result::Ok(public_inputs) => public_inputs,
                Result::Err(_) => {
                    assert(false, LoginErrors::INVALID_PROOF);
                    array![].span() // unreachable
                }
            }
        }
        ...
        fn validate_login_deploy_call(self: @ContractState, call:Call) {
            ...
            if call.selector == selector!("login"){
                ...
                // 移除债务检查，允许有债务的用户 login
                // 债务会在 login 函数中尝试收取，或在下次交易时自动收取
            }
